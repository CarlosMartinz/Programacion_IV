    ...
    private function almacenar_datos(){
        if( $this->respuesta['msg']=='correcto' ){
            if( $this->datos['accion']=='nuevo' ){
                $this->db->consultas('INSERT INTO lectura_agua.clientes(nombre, direccion, zona) 
                    VALUES(?, ?, ?)', 
                    [$this->datos['nombre'], $this->datos['direccion'], $this->datos['zona']]);
                    return $this->db->obtener_ultimo_id();
            }else if( $this->datos['accion']=='modificar' ){
                $this->db->consultas('UPDATE lectura_agua.clientes SET nombre=?, direccion=?, zona=? 
                    WHERE idCliente=?', 
                ...
                $this->db->consultas('DELETE FROM lectura_agua.clientes WHERE idCliente=?', 
